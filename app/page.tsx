...
export default function Page() {
  ...
  const recommendations = useMemo(() => recommendSix(profile, admissions, offset), [profile, admissions, offset]);
  ...
  const handleChange = (patch: Partial<StudentProfile>) => setProfile((prev) => ({ ...prev, ...patch }));
  ...
  return (
    <div className="app"><Header /><main>
      <section className="hero"><Hero onStart={() => document.getElementById("profile")?.scrollIntoView({ behavior: "smooth" })} /><ScorePanel total={competitiveness.total} aiSummary={aiSummary} /></section>
      <section className="section" id="profile"><div className="sectionHead"><div><h2>나의 경쟁력</h2><div className="muted">간단한 성적을 입력하면 실시간으로 다시 계산됩니다.</div></div></div><StudentProfileForm profile={profile} onChange={handleChange} /></section>
      <section className="section"><div className="grid"><CompetitivenessCard label="교과 경쟁력" value={competitiveness.subject} /><CompetitivenessCard label="학종 경쟁력" value={competitiveness.holistic} /><CompetitivenessCard label="수능최저 가능성" value={competitiveness.csatMinimum} /></div></section>
      <section className="section"><div className="sectionHead"><div><h2>🎯 나의 수시 6장</h2><div className="muted">2027 검증 데이터 우선 · 미검증 영역은 mock</div></div><button className="secondary" onClick={() => setOffset((o) => nextShuffleOffset(o))}>조합 다시 짜기</button></div>
        <div className="six">{recommendations.map((rec) => { const { universityName, departmentName } = resolveNames(rec.admissionId); const admission = admissionById.get(rec.admissionId); return <UniversityCard key={rec.admissionId} tier={rec.tier} universityName={universityName} departmentName={departmentName} score={rec.score} reason={`${rec.reason}${admission?.source?.type === "adiga" ? " · 2027 어디가 확인 데이터" : " · 프로토타입 데이터"}`} onShowReason={() => alert(`${universityName} ${departmentName}\n\n${rec.reason}`)} onCompare={() => alert("비교 기능 준비중")} />; })}</div>
      </section>
      ...
      <div className="footer">수시6 v0.2 · 2027 검증 데이터 연동 단계 · 실제 합격을 보장하지 않습니다.</div>
    </main></div>
  );
}
