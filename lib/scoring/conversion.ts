...
export function convertStudentToAdmissionScore(
  ...
  if (admission.type === "교과") {
    ...
    const score = admission.studentRecordWeight != null
      ? quantitative * (1 - weight / 100) + qualitative * (weight / 100)
      : quantitative;
    return {
      score: round(score),
      basis: admission.studentRecordWeight != null
        ? `교과 정량 ${100 - weight}% + 교과 정성 ${weight}%의 프로토타입 환산`
        : "내신 평균등급 기반 프로토타입 환산",
  ...
  if (admission.type === "학종") {
    ...
    return {
      ...
      basis: admission.interview
        ? "학생부 경쟁력 + 면접 가능성을 반영한 프로토타입 환산"
        : "학생부 교과/전공연계 기반 프로토타입 환산",
  ...
  return {
    ...
    basis: "교과·학생부·모의고사를 조합한 프로토타입 환산",
    confidence: 0.25,
  };
}
